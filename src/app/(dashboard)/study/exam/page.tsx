...
export default function PracticeExamPage() {
  ...
  const submitExam = async () => {
    if (timerRef.current) clearInterval(timerRef.current)
    setPhase('results')
    // Record gamification: exam complete
    try {
      await fetch('/api/gamification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'exam_complete' }),
      })
    } catch (e) {
      // Silently fail - don't block exam results
    }
  }
        ...
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 md:py-8">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="card-hover w-full text-left card border-2 border-transparent hover:border-brand-500 p-6 transition-all"
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-3 mt-3 text-xs text-surface-500">
                    <span className="chip-brand">40 {lang === 'sv' ? 'frågor' : 'kysymystä'}</span>
                    <span className="chip-brand">3h</span>
                    <span className="chip-brand">+1 / -0.5</span>
                  </div>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-3 mt-3 text-xs text-surface-500">
                    <span className="chip-brand">{lang === 'sv' ? 'Valfritt antal' : 'Valinnainen määrä'}</span>
                    <span className="chip-brand">{lang === 'sv' ? 'Ingen tidsgräns' : 'Ei aikarajaa'}</span>
                    <span className="chip-brand">{lang === 'sv' ? 'Förklaringar' : 'Selitykset'}</span>
                  </div>
                </div>
              </div>
            </button>
  ...
  if (phase === 'intro' && mode) {
    return (
      <div className="min-h-screen bg-mesh">
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 md:py-8">
          <button onClick={() => setMode(null)} className="text-sm text-surface-500 hover:text-surface-700 mb-6 flex items-center gap-1">
          ...
          <div className="glass p-6 mb-6">
            {mode === 'realistic' ? (
              <>
                <h2 className="font-semibold mb-4 text-surface-900">{lang === 'sv' ? 'Provinformation' : 'Kokeen tiedot'}</h2>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="glass-subtle rounded-lg p-3">
                    <div className="text-surface-500">{lang === 'sv' ? 'Frågor' : 'Kysymyksiä'}</div>
                    <div className="font-bold text-lg text-surface-900">40</div>
                  </div>
                  <div className="glass-subtle rounded-lg p-3">
                    <div className="text-surface-500">{lang === 'sv' ? 'Tid' : 'Aika'}</div>
                    <div className="font-bold text-lg text-surface-900">{lang === 'sv' ? '3 timmar' : '3 tuntia'}</div>
                  </div>
                  <div className="glass-subtle rounded-lg p-3">
                    <div className="text-surface-500">{lang === 'sv' ? 'Typ' : 'Tyyppi'}</div>
                    <div className="font-bold text-lg text-surface-900">{lang === 'sv' ? 'Flerval' : 'Monivalinta'}</div>
                  </div>
                  <div className="glass-subtle rounded-lg p-3">
                    <div className="text-surface-500">{lang === 'sv' ? 'Poäng' : 'Pisteytys'}</div>
                    <div className="font-bold text-lg text-surface-900">+1 / -0.5</div>
                  </div>
                </div>

                <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-200 rounded-lg p-4 mt-4">
              ...
              <>
                <h2 className="font-semibold mb-4 text-surface-900">{lang === 'sv' ? 'Anpassa övningen' : 'Muokkaa harjoittelua'}</h2>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className={`chip-brand text-sm font-medium transition-all ${
                        !selectedArea ? 'ring-2 ring-brand-500' : 'opacity-70 hover:opacity-100'
                      }`}
                    >
                      {lang === 'sv' ? 'Alla' : 'Kaikki'}
                    </button>
                    {Object.entries(areaConfig).map(([key, val]) => (
                      <button
                        ...
                        className={`chip-brand text-sm font-medium transition-all ${
                          selectedArea === key ? 'ring-2 ring-brand-500' : 'opacity-70 hover:opacity-100'
                        }`}
                      >
                        {val[lang as 'fi' | 'sv']}
                      </button>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`chip-brand text-sm font-medium transition-all ${
                          selectedDifficulty === d.value ? 'ring-2 ring-brand-500' : 'opacity-70 hover:opacity-100'
                        }`}
                      >
                        {d[lang as 'fi' | 'sv']}
                      </button>
                ...
                <div>
                  <label className="flex items-center justify-between mb-1">
                    ...
                    <span className="font-medium text-surface-900">{questionCount}</span>
                  </label>
                  <input
                    type="range"
                    min={5}
      ...
      <div className="min-h-screen bg-mesh">
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-6 md:py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <span className="text-sm text-surface-500">{lang === 'sv' ? 'Fråga' : 'Kysymys'}</span>
              <span className="font-bold text-lg ml-1 text-surface-900">
                {currentQ + 1}/{questions.length}
              </span>
            </div>
            {mode === 'realistic' && (
              <div className={`px-4 py-2 rounded-lg font-mono text-lg font-bold transition-all ${
                timeLeft < 600 ? 'bg-red-600 text-white shadow-lg shadow-red-600/50' : 'glass text-surface-900'
              }`}>
                {formatTime(timeLeft)}
              </div>
            )}
          ...
          {/* Progress bar */}
          <div className="glass-subtle rounded-full h-2 mb-6 overflow-hidden">
            <div
          ...
          {/* Question card */}
          <div className="glass p-6 mb-6">
            <div className="text-surface-900 leading-relaxed mb-6">
            ...
            <div className="space-y-3">
              {q.content.options.map((opt, index) => {
                ...
                if (isFreeAnswered) {
                  if (isThisCorrect) {
                    buttonClass += 'glass-subtle border-green-500 bg-gradient-to-r from-green-500/10 to-emerald-500/10'
                  } else if (isThisSelected && !isCorrect) {
                    buttonClass += 'glass-subtle border-red-500 bg-gradient-to-r from-red-500/10 to-rose-500/10'
                  } else {
                    buttonClass += 'glass-subtle border-surface-200 opacity-60'
                  }
                } else if (isThisSelected) {
                  buttonClass += 'glass border-brand-500 ring-2 ring-brand-500/20'
                } else {
                  buttonClass += 'glass-subtle border-surface-200 hover:border-brand-500'
                }
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex-shrink-0 w-8 h-8 rounded-lg glass flex items-center justify-center font-bold text-sm text-surface-600">
                        {optLabels[index]}
                      </div>
            ...
            {mode === 'free' && isAnswered && (
              <div className={`mt-4 p-4 rounded-lg ${isCorrect ? 'bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-200' : 'bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-200'}`}>
          ...
          <div className="flex justify-between gap-3">
            ...
            {currentQ < questions.length - 1 ? (
              ...
              <button
                onClick={submitExam}
                className="btn-primary flex-1 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600"
              >
                {mode === 'realistic'
            ...
            {questions.map((question, i) => (
              <button
                ...
                className={`w-8 h-8 rounded-full text-xs font-medium transition-all ${
                  i === currentQ
                    ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white ring-2 ring-brand-500/20'
                    : answers[question.id]
                    ? 'chip-success'
                    : 'glass-subtle text-surface-600 hover:glass'
                }`}
              >
                {i + 1}
              </button>
  ...
  if (phase === 'results') {
    ...
    return (
      <div className="min-h-screen bg-mesh">
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 md:py-8">
          <div className="text-center mb-8">
            <div className="text-5xl mb-4 animate-fade-in">{percentage >= 70 ? '🎉' : percentage >= 50 ? '💪' : '📚'}</div>
            <h1 className="text-2xl font-bold text-gradient mb-2">
              {lang === 'sv' ? 'Provresultat' : 'Kokeen tulokset'}
            </h1>
            <div className="inline-block mb-1">
              <div className="text-5xl font-bold text-gradient bg-gradient-to-r from-brand-600 to-accent-600">
                {percentage}%
              </div>
            </div>
            {mode === 'realistic' && (
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="card bg-gradient-to-br from-green-500/10 to-emerald-500/10 border border-green-200 rounded-xl p-4 text-center">
            ...
            <div className="card bg-gradient-to-br from-red-500/10 to-rose-500/10 border border-red-200 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-red-700">{results.wrong}</div>
              <div className="text-xs text-red-600">{lang === 'sv' ? 'Fel' : 'Väärin'}</div>
            </div>
            <div className="card bg-gradient-to-br from-surface-500/10 to-slate-500/10 border border-surface-200 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-surface-700">{results.unanswered}</div>
              <div className="text-xs text-surface-600">{lang === 'sv' ? 'Obesvarade' : 'Vastaamatta'}</div>
            </div>
          </div>
          ...
          <button
            ...
            className="card w-full rounded-xl p-4 mb-4 text-left font-medium text-surface-900 flex items-center justify-between hover:border-brand-500 transition-colors"
          >
                  ...
                  <div key={q.id} className="card p-4">
                    <div className="flex items-start gap-3 mb-3">
                      <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                        wasUnanswered ? 'bg-surface-100 text-surface-500' : isCorrectAnswer ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
                        {i + 1}
                      </span>
                      <div className="flex-grow">
                        <div className="text-sm text-surface-900 mb-2">
                          <MathRenderer content={getQuestionText(q)} />
                        </div>
                        {!wasUnanswered && (
                        ...
                        {(!isCorrectAnswer || wasUnanswered) && (
                          <div className="mt-2 text-xs text-blue-800 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 p-2 rounded border border-blue-200">
                            <MathRenderer content={getExplanationText(q)} />
                          </div>
                        )}
                      </div>
